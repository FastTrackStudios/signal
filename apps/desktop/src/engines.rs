...
/// What and where the signal engine is.
///
/// `engine_launcher` supplies the generic machinery — find the binary, spawn
/// it, probe the port, talk to systemd — but the *identity* of this engine is
/// ours: our binary name, our port, our unit. That const used to live in the
/// daw repo, which meant renaming our own binary needed a tag in someone
/// else's tree. It belongs with the thing it describes.
///
/// `package` is both the executable name looked for on disk and the cargo
/// package for the dev fallback (`cargo run -p …`), so the two must stay
/// equal to the crate's `[[bin]]` name.
///
/// `unit` stays `signal-engine` rather than tracking the package name: it is
/// the name of a service users already have installed, and renaming it would
/// orphan a running unit for no gain.
...
use engine_launcher::{
    Engine, KillSignal, LaunchSource, kill_group, probe, spawn, systemd_active, systemd_available,
    systemd_start, systemd_stop,
};
...
pub fn start_signal() -> Result<String, String> {
    ...
    let how = match &spawned.source {
        ...
        LaunchSource::Cargo => "cargo run -p signal-desktop -- --engine (dev fallback)".into(),
    };
    tracing::info!("signal engine started (pid {}) via {how}", spawned.pid());
    let url = spawned.ws_url.clone();
    *OWNED
        .lock()
        .unwrap_or_else(std::sync::PoisonError::into_inner) = Some(spawned.child);
    Ok(url)
}
