...
use midicore::{
    Channel, ControllerNumber, ControllerValue, DrumMap, DrumMapConverter, KeyNumber, MidiEvent,
    Velocity,
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let preset =
        std::path::PathBuf::from(std::env::var("SIGNAL_PRESET").unwrap_or_else(|_| PRESET.into()));
    if !preset.exists() {
    ...
    loop {
        ...
        for id in &ids {
            let (a, b) = rig.preload_progress(id);
            l += a;
            t += b;
        }
        if t > 0 && l >= t {
    ...
    let dispatch = |rig: &SamplerRig, e: &MidiEvent| match e {
        ...
        MidiEvent::ControlChange {
            controller, value, ..
        } => rig.midi_message(CH, 0xB0, controller.get(), value.get()),
        _ => {}
    };
    ...
    while pos < total {
        while i < sched.len() && sched[i].0 < pos + BLK {
            let raw = match &sched[i].1 {
                Hit::Note(n, v) => MidiEvent::NoteOn {
                    channel: ch,
                    key: KeyNumber::new(*n),
                    velocity: Velocity::new(*v),
                },
                Hit::HatCc(v) => MidiEvent::ControlChange {
                    channel: ch,
                    controller: ControllerNumber::new(4),
                    value: ControllerValue::new(*v),
                },
            };
            for out in conv.convert(raw) {
                dispatch(&rig, &out);
                if matches!(out, MidiEvent::NoteOn { .. }) {
                    emitted += 1;
                }
            }
            i += 1;
    ...
    if let Some(d) = outp.parent() {
        std::fs::create_dir_all(d).ok();
    }
    write_wav(outp, &out_buf)?;
    println!("wrote {}", outp.display());
    if peak < 1e-3 {
        eprintln!("WARNING: silent output");
        std::process::exit(2);
    }
    Ok(())
}
