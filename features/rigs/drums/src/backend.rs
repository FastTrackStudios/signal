...
impl DrumRigBackend {
    ...
    pub fn new() -> Self {
        let library_dir = std::env::var("SIGNAL_DRUM_LIBRARY")
            .map_or_else(|_| PathBuf::from(DEFAULT_LIBRARY), PathBuf::from);
    ...
    fn apply_kit_mixer(&self) {
        ...
        for (bi, bus) in mix.buses.iter().enumerate() {
            let sender_solo = mix.sends.iter().any(|snd| {
                snd.bus == bi
                    && (snd.soloed || mix.pieces.get(snd.piece).is_some_and(|p| p.state.soloed))
            });
    ...
    fn status(&self) -> DrumStatus {
        ...
        if running {
            ...
            if let Some(rig) = rig.as_ref() {
                ...
                let peak = |idx: usize| meters.cell(idx).map_or(0.0, |c| c.peak(0).max(c.peak(1)));
