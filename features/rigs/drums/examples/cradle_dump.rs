...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for s in &mixer.strips {
        ...
        let flag = if s.mute { " [MUTE]" } else if s.solo { " [SOLO]" } else { "" };
        println!("── {:<14} {:>6.1} dB  pan {:>5.2}{}", s.name, db, s.pan, flag);
        for fx in s.fx_slots() {
            ...
            let detail = match fx.fx_type.as_str() {
                ...
                "Modern Compressor" | "Vintage Compressor" => format!(
                    ...
                    fx.num("threshold").map(|v| format!("{v:.1}")).unwrap_or_default(),
                    fx.num("ratio").map(|v| format!("{v:.2}")).or_else(|| fx.text("ratio").map(str::to_string)).unwrap_or_default(),
                    fx.num("attack").map(|v| format!("{v:.3}")).or_else(|| fx.text("attack").map(str::to_string)).unwrap_or_default(),
                    fx.num("release").map(|v| format!("{v:.3}")).or_else(|| fx.text("release").map(str::to_string)).unwrap_or_default(),
                    fx.text("knee").unwrap_or(""),
                    fx.num("mix").unwrap_or(1.0),
                ),
                "Transient" => format!(
                    "attack {:.3} sustain {:.3} mix {:.2}",
                    fx.num("attack").unwrap_or(0.0), fx.num("sustain").unwrap_or(0.0), fx.num("mix").unwrap_or(1.0)
                ),
                "Drive" => format!("drive {:.2} mode {:?}", fx.num("drive").unwrap_or(0.0), fx.text("mode").unwrap_or("")),
                "Reverb" => format!(
                    "mode {:?} decay {:.2} pre {:.3} hp {:.2} lp {:.2}",
                    fx.text("mode").unwrap_or(""), fx.num("decay").unwrap_or(0.0),
                    fx.num("preDelay").unwrap_or(0.0), fx.num("hipass").unwrap_or(0.0), fx.num("lopass").unwrap_or(0.0)
                ),
