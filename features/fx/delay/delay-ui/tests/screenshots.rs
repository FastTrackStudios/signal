...
//! ```sh
//! just delay-shots      # or:
//! cargo test -p delay-ui --features native --test screenshots
//! ```
//!
//! Output lands in `target/gui-shots/delay/` (`FTS_SHOTS_DIR` overrides).
...
fn shots_dir() -> PathBuf {
    let dir = std::env::var("FTS_SHOTS_DIR").map_or_else(
        |_| PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../../../target/gui-shots/delay"),
        PathBuf::from,
    );
...
async fn on_profile(profile_id: &str) -> Fixture {
    let params = std::sync::Arc::new(delay_ui::params::DelayParams::default());
    params.store_profile_id(delay_profiles::profile_index(profile_id).unwrap());
