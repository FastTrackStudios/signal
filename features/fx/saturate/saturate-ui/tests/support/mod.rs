//! Shared fixture for the saturate editor's headless tests.
//!
//! Mounts `saturate_ui::control_view::App` — the exact Dioxus surface the
...
pub fn mount_sized(width: u32, height: u32) -> Fixture {
    mount_with(Arc::new(SatParams::default()), width, height)
}
...
pub fn mount_with(params: Arc<SatParams>, width: u32, height: u32) -> Fixture {
    let ui_state = Arc::new(SatUiState::default());
    ...
    let tester = render(Harness)
        ...
        .with_root_context(SharedState::new(Arc::new(SatUi {
            params: params.clone(),
