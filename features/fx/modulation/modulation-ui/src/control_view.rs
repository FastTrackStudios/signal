//! The modulation editor: a rail of five circuits, and the panel of whichever
//! one is selected.
//!
//! Same shell as the saturator, deliberately — the two plugins are the same
//! instrument as far as your hands are concerned. Clicking the family you are
//! already on cycles the machines inside it.
...
/// Root editor component. Takes no props — the plugin puts [`ModUi`] in
...
pub fn App() -> Element {
    ...
    let ui = shared
        .get::<ModUi>()
        .expect("the editor was mounted without its ModUi");
    ...
    if last.get() != Some((profile_index, form)) {
        ...
        if params.profile.value().max(0) as usize != profile_index {
            ...
            let count = modulation_profiles::PROFILES.len();
    ...
    let handles: HashMap<String, ParamHandle> = [
        ("rate", params.rate.as_ptr()),
        ("depth", params.depth.as_ptr()),
        ("mix", params.mix.as_ptr()),
        ("output", params.output.as_ptr()),
        ("knob_a", params.knob_a.as_ptr()),
        ("knob_b", params.knob_b.as_ptr()),
        ("knob_c", params.knob_c.as_ptr()),
        ("knob_d", params.knob_d.as_ptr()),
    ]
    .into_iter()
    ...
    let active_category = modulation_profiles::category_of(profile.id)
        .map(|(c, _)| c)
        .unwrap_or(0);
    let items: Vec<ShellItem> = modulation_profiles::CATEGORIES
        .iter()
        .enumerate()
        .map(|(index, category)| {
            ...
            let at = if index == active_category {
                modulation_profiles::category_of(profile.id).map(|(_, v)| v).unwrap_or(0)
            } else {
                0
            };
    ...
    let profile_count = modulation_profiles::PROFILES.len();
    ...
    rsx! {
        ThemeProvider { state: theme,
            ...
            DragProvider {
            PluginShell {
                title: "FTS Modulation".to_string(),
                ...
                on_select: move |category: usize| {
                    let index = modulation_profiles::rail_click_target(profile_index, category);
...
mod tests {
    ...
    fn every_profile_has_a_badge_that_fits_the_rail() {
        for profile in modulation_profiles::PROFILES {
            ...
            assert_ne!(badge, "MOD", "{} fell through to the default badge", profile.id);
        }
    }

    #[test]
    fn every_family_has_a_panel() {
        for category in modulation_profiles::CATEGORIES {
            for id in category.profiles {
                ...
                assert!(
                    !crate::faces::placed_controls(id).is_empty(),
                    "{id} has no controls",
                );
            }
        }
