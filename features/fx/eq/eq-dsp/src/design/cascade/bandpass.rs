...
use super::{
    bell_three_point_synth, bp_cascade_for_q, lagrange_synth_alt_path, mode0_forward,
    notch_analog_sections, proq4_s2_from_prototype_with_subfreq,
};
...
#[expect(
    ...
    reason = "Pre-computed omega powers and trig values grouped for efficiency"
)]
fn bandpass_cascade_section(
    ...
    if matches!(slope, 5..=9) && freq_hz >= bp_lagrange_threshold && !(slope == 5 && sec_idx == 2) {
    ...
    let p3 = if w_section_raw <= PI {
        if slope == 3 && a2_sec > 1.0 {
            ...
            let hf = ((freq_hz - 15000.0) / 7000.0).clamp(0.0, 1.0);
            let hf_tail_scale =
                ((2.00 * (0.40_f64.mul_add(q_hi, 0.60))).mul_add(-hf, 1.0)).max(0.005);
                ...
                (0.004_f64.mul_add(
                    ...
                    0.00045_f64.mul_add(
                        ...
                        0.00045_f64.mul_add(
                            ...
                            0.009_f64.mul_add(
                                mid_lift,
                                (0.1040_f64 * x).mul_add(-x, 0.0928_f64.mul_add(x, 1.0078)),
                            ),
                        ),
                    ),
                ))
            ...
            (0.00016_f64 * low).mul_add(
                q_mid,
                ((sp6 / SQRT_2) * hf_tail_scale * (0.034_f64 * low).mul_add(q_one, 1.0))
                    .mul_add(q_half_tail, 0.45 * omega0 * omega0 / q_user)
        ...
        let r_eval = if t_eval2 > 1e-300 {
            ...
            h_sq(w_eval) * d2 * dz_mag_sq(w_eval) * one_plus_t2 * one_plus_t2 / (16.0 * t_eval2)
        } else {
            0.0
        };
...
fn bp_s3_tail_p3_for_q(q: f64) -> f64 {
    ...
    let q = q.max(0.5);
    ((0.708_282_359_681_502_1_f64 * q).mul_add(
        q,
        0.071_551_196_033_465_8_f64.mul_add(q, 0.047_750_333_818_323),
    )) / (q * q - 0.227_708_022_498_569_14 * q)
}
fn bp_s3_tail_p2_for_q(q: f64) -> f64 {
    let q = q.max(0.5);
    ((0.706_361_059_557_113_f64 * q).mul_add(
        q,
        -(0.617_222_169_155_086_7_f64 * q) + 0.127_302_505_126_316_7,
...
fn bp_s5_tail_p3_for_q(q: f64, omega0: f64) -> f64 {
    ...
    phi_conj
        + (0.014_f64.mul_add(q01, 0.1398 + 0.0005_f64.mul_add(low_omega, 0.0))) * omega0 * omega0
            / (q * q)
}
