...
impl<S: SignalApi> BlockPresetOps<S> {
    pub async fn list(&self, block_type: BlockType) -> Vec<Preset> {
        let cx = self.0.context_factory.make_context();
        self.0.service.list_block_presets(&cx, block_type).await
    }
    ...
    pub async fn save(&self, preset: Preset) -> Preset {
        let cx = self.0.context_factory.make_context();
        self.0.service.save_block_preset(&cx, preset.clone()).await;
        preset
    }

    pub async fn delete(&self, block_type: BlockType, preset_id: impl Into<PresetId>) {
        let cx = self.0.context_factory.make_context();
        self.0
            .service
