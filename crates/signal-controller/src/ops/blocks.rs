...
impl<S: SignalApi> BlockOps<S> {
    pub async fn get(&self, block_type: BlockType) -> Block {
        let cx = self.0.context_factory.make_context();
        self.0.service.get_block(&cx, block_type).await
    }

    pub async fn set(&self, block_type: BlockType, block: Block) -> Block {
        let cx = self.0.context_factory.make_context();
        self.0.service.set_block(&cx, block_type, block).await
    }

    pub async fn get_value(&self, block_type: BlockType) -> f32 {
        self.get(block_type).await.first_value().unwrap_or(0.0)
    }
