...
define_class!(
    ...
    // FTS PATCH — the `#[name = "RawWindowMetalLayer"]` that was here is
    // removed so objc2 auto-generates the runtime name.
    //
    // An explicit name must be unique across the whole PROCESS. Every FTS
    // plugin bundle statically links its own copy of this crate, so the second
    // plugin to create a Metal surface tried to register a class name the
    // first one already owned, and objc2 panicked with `class_not_unique`.
    // The panic unwound through an AppKit block — "terminating due to uncaught
    // foreign exception" — and took the host down with it. Loading FTS EQ and
    // FTS Comp into one REAPER was enough to do it, which made the whole suite
    // unusable together.
    //
    // objc2's own `define_class!` documentation prescribes exactly this fix:
    // "if you're developing a library, it is recommended that you do not set
    // this ... if the name is auto-generated, the class will also be allowed to
    // be used across multiple shared dynamic libraries in the same process."
    #[ivars = Weak<CALayer>]
